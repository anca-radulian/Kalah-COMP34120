...
class Board:
    ...
    """
    __board = [[0 for x in range(2)] for y in range(1)]
...
def clone(self):
    return copyBoard(self, self.board)
...
def getSeeds(self, side, hole):
    if hole < 1 or hole > self.holes:
        raise Exception(
            "Hole number must be between 1 and " + (self.board[self.NORTH_ROW].length - 1) + " but was " + hole + ".")

    return self.board[indexOfSide(self, side)][hole]
...
def setSeeds(self, side, hole, seeds):
    ...
    self.board[indexOfSide(self, side)][hole] = seeds
    self.setChanged()
...
def getSeedsOp(self, side, hole):
    ...
    return self.board[1 - indexOfSide(self, side)][self.holes + 1 - hole]
...
def setSeedsOp(self, side, hole, seeds):
    ...
    self.board[1 - indexOfSide(self, side)][self.holes + 1 - hole] = seeds
    self.setChanged()
...
def getSeedsInStore(self, side):
    return self.board[indexOfSide(self, side)][0]
...
def setSeedsInStore(self, side, seeds):
    ...
    self.board[indexOfSide(self, side)][0] = seeds
    self.setChanged()
...
def __str__(self):
    ...
    boardString += self.board[self.NORTH_ROW][0] + "  --"
    for i in range(self.holes, 0, -1):
        boardString += "  " + self.board[self.NORTH_ROW][i]
    boardString += "\n"
    for i in range(1, self.holes + 1, 1):
        boardString += self.board[self.SOUTH_ROW][i] + "  "
    boardString += "--  " + self.board[self.SOUTH_ROW][0] + "\n"
